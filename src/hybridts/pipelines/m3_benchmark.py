"""Unified M3 benchmark pipeline.

Runs all models (classical + neural) on M3 dataset with 6 metrics.
Replaces the old Helformer-centric and hybrid-centric pipelines.
"""
...
from ..data import (
    M3_H, M3_P, best_L, ensure_m3_csv, load_train_tsts,
...
def run_m3_benchmark(
    ...
    """Run a full M3 benchmark.

    Parameters
    ----------
    categories : sequence of str
        M3 frequency categories to evaluate.
    n_per_cat : int
    ...
    force_rebuild_csv : bool
        Force rebuild of M3 CSV files from TSF.

    Returns
    -------
    pd.DataFrame with per-series, per-model metrics.
    """
    out_path = Path(out_dir or (settings.outputs_dir / "m3_benchmark"))
    out_path.mkdir(parents=True, exist_ok=True)

    ensure_m3_csv(force_rebuild=force_rebuild_csv)
    ...
    for cat in _progress(categories, desc="M3 categories"):
        H = M3_H[cat]
        P = M3_P[cat]
